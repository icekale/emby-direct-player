...
const src = fs.readFileSync(
    new URL("./direct_module.js", import.meta.url),
    "utf8",
);
const EXPORTS = [
    "dpParseVars",
...
async function loadModule(stubs) {
    ...
    const decl = Object.keys(stubs)
        .map((n) => `const ${n} = globalThis.__stub.${n};`)
        .join("\n");
    fs.writeFileSync(
        file,
        `${decl}\n${src}\nexport { ${EXPORTS.join(", ")} };`,
...
const ApiClient = {
    ...
    getUrl: (p, q) =>
        "http://srv" + p + (q ? "?" + new URLSearchParams(q) : ""),
    ajax: async (o) => {
        let body;
        try {
            ...
            throw new Error(
                `自检桩: 请求体不是合法 JSON (${e.message}): ${String(o.data).slice(0, 200)}`,
            );
        }
...
const mkEl = (cls, tag = "button") => {
    const node = {
        ...
        addEventListener() {},
        contains(n) {
            return n === this || this.children.includes(n);
        },
        // 真 DOM 的 append 接受字符串（当文本节点），桩也照做
        append(...nodes) {
            for (const n of nodes) {
                if (typeof n === "string") {
                    node.children.push({ nodeType: 3, textContent: n });
                    continue;
                }
                n.parent = node;
        ...
        insertAdjacentElement(_pos, n) {
            // 真页面里只用到 'afterend'，桩就固定插在节点后面
            const p = node.parent,
                i = p.children.indexOf(node);
    ...
    Object.defineProperty(node, "innerHTML", {
        get: () => node.children.map(serEl).join(""),
        set: () => {},
    });
    Object.defineProperty(node, "nextElementSibling", {
        get() {
            const p = node.parent;
            return p ? p.children[p.children.indexOf(node) + 1] || null : null;
        },
    });
    return node;
};
...
for (const el of [
    playBtn,
    mkEl("button btnResume raised button-submit block emby-button"),
    mkEl("button btnDownload raised button-submit block emby-button"),
    mkEl("button detailButton raised button-submit block emby-button"),
]) {
...
assert.equal(
    M.dpPlayingOther(
        "http://h/emby/Videos/abc123/stream?Static=true&api_key=t",
        "abc123",
    ),
    false,
);
assert.equal(
    M.dpPlayingOther("http://h/emby/Videos/other/stream", "abc123"),
    true,
);
assert.equal(
    M.dpPlayingOther("http://h/emby/Videos/abc%31%32%33/stream", "abc123"),
    false,
); // 带百分号编码的路径
...
assert.equal(
    M.dpItemIdFromHash(
        "#!/item?id=4405927&serverId=2aabaaa9a6834271a69ec9a4afa4a394&context=home",
    ),
    "4405927",
);
...
assert.equal(
    M.dpItemIdFromUrl(
        "http://10.0.0.9:8096/Videos/2f3c0a1b2c3d4e5f6a7b8c9d0e1f2a3b/stream?Static=true&api_key=x",
    ),
    "2f3c0a1b2c3d4e5f6a7b8c9d0e1f2a3b",
);
assert.equal(
    M.dpItemIdFromUrl(
        "http://h/emby/audio/2f3c0a1b2c3d4e5f6a7b8c9d0e1f2a3b/stream.mp3?a=1",
    ),
    "2f3c0a1b2c3d4e5f6a7b8c9d0e1f2a3b",
);
...
assert.ok(
    M.dpStreamUrl("4405927", ms, pb).startsWith(
        "http://10.0.0.9:8096/emby/Videos/4405927/stream.mkv?",
    ),
);
...
// 直链主机留空 + auto → 用原始链接
assert.equal(
    M.dpStreamUrl("4405927", strmMs, pb),
    "http://10.0.0.9:9527/Media/xxx.mkv",
);
// 直链主机有值 → auto 不用原始链接
localStorage.setItem("directPlayerStreamBase", "http://10.0.0.9:8096");
assert.ok(
    M.dpStreamUrl("4405927", strmMs, pb).startsWith(
        "http://10.0.0.9:8096/Videos/4405927/stream.mkv?",
    ),
);
...
assert.ok(
    M.dpStreamUrl(
        "4405927",
        { ...ms, Path: "/mnt/media/x.mkv" },
        pb,
    ).startsWith("http://10.0.0.9:8096/Videos/"),
);
localStorage.setItem("directPlayerUrlFrom", "auto");
localStorage.setItem("directPlayerStreamBase", "");

// ---- 默认配置 ----
assert.deepEqual(M.dpConf(), {
    mode: "jriver",
    jriver: "http://127.0.0.1:52199",
    jrkey: "",
    zone: "-1",
    mpc: "http://127.0.0.1:13579",
    streamBase: "",
    urlFrom: "auto",
});
...
const run = async (polls, over = {}) => {
    calls.length = 0;
    await M.dpMonitor({ ...ctx, ...over }, async () =>
        polls.length ? polls.shift() : { gone: true },
    );
    return calls.map(
        (c) =>
            `${c.url.split("/").pop()}:${c.PositionTicks}${c.IsPaused ? ":paused" : ""}`,
    );
};
...
assert.deepEqual(
    await run([
        { playing: true, posMs: 60000 },
        { playing: true, posMs: 63000 },
        { playing: true, posMs: 66000 },
        { playing: true, posMs: 69000 },
        { playing: true, posMs: 72000 },
    ]),
...
assert.deepEqual(
    await run(
        [
            { playing: false, paused: true, posMs: 1000 },
            { playing: false, paused: true, posMs: 1000 },
            { playing: false, paused: true, posMs: 1000 },
            { playing: false, paused: true, posMs: 1000 },
            { gone: true },
...
assert.deepEqual(
    await run([{ playing: true, posMs: 5000 }, null, null, null], {
        resumeMs: 0,
    }),
...
// 播完（位置 >= 片长）上报停止
calls.length = 0;
await M.dpMonitor({ ...ctx, runMs: 10000, resumeMs: 0 }, async () => ({
    playing: true,
    posMs: 11000,
}));
assert.deepEqual(
...
assert.equal(
    M.dpRelayHref("mpc", "http://h/a.mkv", 0),
    "etlp-mpc:http%3A%2F%2Fh%2Fa.mkv",
);
assert.equal(
    M.dpRelayHref("mpv", "http://h/a b.mkv", 90500),
    "etlp-mpv:http%3A%2F%2Fh%2Fa%20b.mkv||90500",
);
assert.equal(
    M.dpRelayHref("mpc", "http://h/a.mkv", 20000),
    "etlp-mpc:http%3A%2F%2Fh%2Fa.mkv",
); // <30s 不值得定位

// ---- 详情页三按钮：沿用原生按钮的 class（样式匹配），插在原生播放按钮那一簇之后 ----
assert.equal(
    M.dpButtonBaseClass("button btnPlay raised item-tag-button emby-button"),
    "button raised item-tag-button emby-button",
);
assert.equal(M.dpButtonBaseClass(""), "raised button-submit block emby-button");
M.dpSyncDetailButtons();
const ourBtns = row.children.filter(
    (c) => c.id && c.id.startsWith("etlp-dp-btn-"),
);
assert.deepEqual(
...
); // 在 ▶/继续/下载 之后、⋯ 之前，没拆开原生按钮
assert.deepEqual(
    ourBtns.map((b) => b.title.replace("直连播放：", "")),
    ["MPC-HC", "JRiver", "MPV"],
);
assert.equal(
    ourBtns[0].innerHTML,
    '<i class="md-icon button-icon button-icon-left">theaters</i>MPC-HC 播放',
);
assert.equal(
    ourBtns[0].className,
    "button raised button-submit block emby-button",
); // 抄了原生 class，但没抄 btnPlay
M.dpSyncDetailButtons(); // 幂等：定时器每 1.5s 都会调它
assert.equal(
    row.children.filter((c) => c.id && c.id.startsWith("etlp-dp-btn-")).length,
    3,
);
...
assert.deepEqual(
    created,
    [],
    "不该有 logger.error: " + JSON.stringify(created.slice(0, 3)),
);
