    ...
    const dpTrim = s => String(s || '').trim().replace(/\/+$/, '');
    ...
    const dpIntercept = () => dpEnabled() || localStorage.getItem(etlpStorageKeys.webPlayerEnable) != 'true';
    const dpConf = () => ({
    ...
    function dpParseVars(html) {
        ...
        const re = /id="([^"]+)"[^>]*>([^<]*)</g;
        let m;
        while ((m = re.exec(html || '')) !== null) { out[m[1]] = m[2].trim(); }
        return out;
    }
    ...
    function dpTimeSyntax(ms) {
        ...
        return [Math.floor(ms / 3600000), Math.floor(ms % 3600000 / 60000), Math.floor(ms % 60000 / 1000), ms % 1000].join(':');
    }
    ...
    function dpXmlItems(text) {
        ...
        try {
            ...
            for (const it of doc.getElementsByTagName('Item')) { out[it.getAttribute('Name')] = it.textContent; }
            out.__status = doc.documentElement ? (doc.documentElement.getAttribute('Status') || '') : '';
            out.__error = doc.documentElement ? (doc.documentElement.getAttribute('Error') || '') : '';
        } catch (e) { logger.error('直连: MCWS 解析失败', e); }
        return out;
    }
    ...
    function dpJriverCall(conf, path, params = {}) {
        const q = new URLSearchParams({ ...params, ...dpJrAuthParams(conf), Zone: conf.zone });
    ...
    const dpItemIdFromUrl = url => (String(url).match(/\/([\w-]{16,})\//) || [])[1] || '';
    ...
    async function dpJriverStart(conf, url, itemId) {
        ...
        if (dpStore.get(dpKeys.jrNoUrl, '') === host) return { err: '（这个地址的直链这台 MC 收不了，直接走对话框）' };
        const attempts = [
        ...
        for (const [path, params] of attempts) {
            ...
            if (!dpMcwsOk(r)) { errs.push(`${path}=${(d.__error || 'HTTP ' + r.status).slice(0, 60)}`); continue; }
            for (let i = 0; i < 4 && itemId; i++) {   // 「真的在播了」才算成功
                if (await dpJriverPlaying(conf, itemId)) { logger.info('直连: JRiver 起播方式', path); return { how: path }; }
                await dpSleep(700);
            }
    ...
    function dpUrlHost(url) { try { return new URL(url).origin; } catch { return ''; } }
    ...
    const dpProbeList = [
        ...
        ['Playback/PlayByFilename', { Filenames: '{url}', Filename: '{url}', Location: 'End' }],
    ...
    async function dpJriverProbe() {
        ...
        if (!url) return alert('先在 Emby 里点一次播放（失败没关系），脚本要那条直链才能试。');
        ...
        if (preMcc) {   // 先开「打开 URL」窗口，再试键位写法
            await dpJriverCall(conf, 'Control/MCC', { Command: preMcc });
            await dpSleep(1500);
        }
        const r = await dpJriverCall(conf, path, params);
        const d = dpXmlItems(r.text);
        let txt = `第 ${i + 1}/${dpProbeList.length} 种：${path}\n参数：${JSON.stringify(params)}\n`
        ...
        if (dpMcwsOk(r)) {
            ...
            let played = false;
            for (let k = 0; k < 4 && id && !played; k++) { played = await dpJriverPlaying(conf, id); if (!played) await dpSleep(700); }
    ...
    function dpJriverPoller(conf, itemId) {
        ...
        return async () => {
            ...
            if (lastPos >= 0 && posMs > lastPos && playingValue === null) playingValue = String(d.State);
            lastPos = posMs;
            const playing = playingValue === null ? posMs > 0 : String(d.State) === playingValue;
            return { playing, paused: !playing, posMs, durMs: Number(d.Duration) || 0 };
        };
    }
    ...
    async function dpJriverSeek(conf, ms) {
        if (ms < 30000) return;
        let loaded = '';
        for (let i = 0; i < 60; i++) {   // 给到 30 秒：对话框路线下用户还要手动粘一下
            const r = await dpJriverCall(conf, 'Playback/Info');
            if (r.ok) loaded = dpXmlItems(r.text).Filename || '';
            if (loaded) break;
            await dpSleep(500);
        }
        if (!loaded) { logger.info('直连: 没等到 MC 开始播，跳过续播定位'); return; }
        const r = await dpJriverCall(conf, 'Playback/Position', { Position: String(Math.round(ms)) });
        if (!dpMcwsOk(r)) logger.error('直连: JRiver 续播定位失败(可忽略)', r.status, (r.text || '').slice(0, 200));
    }
    ...
    const dpMpcPoll = async (conf, itemId) => {
        ...
        const out = { posMs: Number(d.position) || 0, durMs: Number(d.duration) || 0 };
        const state = Number(d.state);
        if (state === 2) { out.playing = true; }
        else if (state === 1) { out.playing = false; out.paused = true; }
        else if (state === 0) { out.playing = false; out.stopped = true; }
        else { out.idle = true; }        // -1 = 还没加载媒体
        return out;
    };
    ...
    async function dpMpcSeek(conf, ms) {
        ...
        const q = new URLSearchParams({ wm_command: '-1', position: dpTimeSyntax(ms) });
    ...
    function dpStreamUrl(itemId, ms, playbackData) {
        ...
        if (src && /^https?:/i.test(src) && (conf.urlFrom === 'path' || (conf.urlFrom === 'auto' && !conf.streamBase))) return src;
        const token = ApiClient?._userAuthInfo?.AccessToken || ApiClient?._serverInfo?.AccessToken || '';
        const ext = String(ms?.Container || '').split(',')[0].trim();
        ...
        const tail = basePath && pagePath.startsWith(basePath) ? pagePath.slice(basePath.length) : pagePath;
        return `${base}${tail}?${new URLSearchParams(params)}`;
    }

    async function dpReport(ctx, kind, posMs, isPaused) {
        const path = { start: '/Sessions/Playing', progress: '/Sessions/Playing/Progress', stop: '/Sessions/Playing/Stopped' }[kind];
        const body = {
            ItemId: ctx.itemId,
        ...
        if (kind === 'progress') { body.EventName = 'timeupdate'; body.IsPaused = Boolean(isPaused); }
        try {
            await ApiClient.ajax({ type: 'POST', url: ApiClient.getUrl(path), data: JSON.stringify(body), contentType: 'application/json' });
    ...
    async function dpMonitor(ctx, poll) {
        ...
        while (Date.now() < deadline) {
            await dpSleep(3000);
            const st = await poll();
            if (st === null || st.idle) {                 // 播放器没响应 / 还没加载
                miss++;
            ...
            if (st.stopped) { if (playedOnce) break; continue; }   // MPC-HC 里按了停止
    ...
    async function dpPlay(playbackData, extraData, itemId) {
        ...
        const ctx = {
            ...
            resumeMs: Math.round((main?.UserData?.PlaybackPositionTicks || 0) / 10000),
        };
        ...
        if (mode === 'jriver') {
            ...
            if (!started.how) {
                ...
                if (!copied) prompt('直链（Ctrl+C 复制 → 到 JRiver 窗口 Ctrl+V 回车）', url);   // 复制不到才让用户手拿，且每次都给
                if (firstTime) {
                    alert(`JRiver 没能自动起播：${started.err}\n\n${copied ? '直链已经复制到剪贴板' : '直链在上一个输入框里（Ctrl+C 复制）'}；JRiver 的「打开 URL」窗口已经帮你弹出 → 在里面 Ctrl+V、回车。\n（同一个直链主机只弹一次这个说明；右下角小提示会一直在）\n想完全不手工：把模式换成 mpc。`);
                } else if (copied) {
                    playNotifiy('JRiver 需要按一下 Ctrl+V', '直链已在剪贴板，MC 的「打开 URL」框已弹出：Ctrl+V 回车');
                }
            }
            // 对话框路线也继续监控：粘完开始播之后进度回传/续播定位照旧生效
            dpMonitor(ctx, dpJriverPoller(conf, itemId)).catch(e => logger.error('直连: 监控异常', e));
            dpJriverSeek(conf, ctx.resumeMs).catch(e => logger.error('直连: 续播异常', e));
            return true;
        }
        ...
        if (mode === 'mpc' || mode === 'mpv') {
            ...
            if (!isMpv) {   // MPV 没有网页界面，只有 MPC-HC 需要先开它（回传进度/续播定位靠它）
                const alive = await dpHttp(`${conf.mpc}/variables.html`, { timeout: 2500 });
                if (!alive.ok) {
                    alert(`MPC-HC 网页界面 (${conf.mpc}) 打不开，先启动 MPC-HC 并开启：选项 → 网页界面 → 勾选「监听端口 ${conf.mpc.split(':').pop()}」。\n（回传进度靠这个接口，所以必须先打开。）`);
                    return false;
                }
            }
            playNotifiy(`正在播放 (${who})`, main.Name || '');
            const copied = isMpv ? dpCopyText(url) : false;   // MPV 没有可查的网页接口，中继没装/找不到 mpv.exe 时靠剪贴板当退路
            if (copied) playNotifiy('MPV 没自动起来的话', '直链已在剪贴板：到 mpv 窗口按 Ctrl+V 回车（会自动弹一次打开文件框）');
            ...
            if (isMpv) {
                ...
                dpMonitor(ctx, () => dpMpcPoll(conf, itemId)).catch(e => logger.error('直连: 监控异常', e));
                dpMpcSeek(conf, ctx.resumeMs).catch(e => logger.error('直连: 续播异常', e));
            }
            return true;
        }
    ...
    function dpShowConfig() {
        const cur = dpConf();
        const mode = prompt('直连播放器模式（详情页那三个按钮不受这里影响，它们各自指定播放器）：\njriver = JRiver Media Center（MCWS）\nmpc = MPC-HC（需先装 etlp-mpc 协议中继）\nmpv = MPV（需装同一个中继，它会连 etlp-mpv 也一起注册）', cur.mode);
        if (mode !== null) dpStore.set(dpKeys.mode, mode.trim().toLowerCase());
        dpStore.set(dpKeys.jrNoUrl, '');   // 设置改过就重试一次 MCWS 直链起播
        const jr = prompt('JRiver MCWS 地址（Library Server，默认端口 52199）', cur.jriver);
        if (jr !== null) dpStore.set(dpKeys.jriver, dpTrim(jr));
        const jrkey = prompt('JRiver 访问秘钥（选项 → 媒体网络 里的 Access Key，六位字母；没有认证就留空）', cur.jrkey);
        if (jrkey !== null) dpStore.set(dpKeys.jrkey, jrkey.trim());
        const zone = prompt('JRiver Zone（-1 = 当前区域；也可填 0/1/2 或区域名）', cur.zone);
        if (zone !== null) dpStore.set(dpKeys.zone, zone.trim());
        const mpc = prompt('MPC-HC 网页界面地址（选项 → 网页界面 里的监听端口，默认 13579）', cur.mpc);
        if (mpc !== null) dpStore.set(dpKeys.mpc, dpTrim(mpc));
        const sbase = prompt('直链主机（留空 = 跟着当前网页地址走）\n什么时候需要改：网页走反代端口时，从那里拿到的直链可能 302 到网盘 CDN（外部播放器多半不吃），改成 Emby 自己的端口（例：http://192.168.1.10:8096）更稳\n（路径前缀 /emby 会自动跟着网页走，不用写）', cur.streamBase);
        if (sbase !== null) dpStore.set(dpKeys.streamBase, dpTrim(sbase));
        const from = prompt('直链来源：auto = 下面两空才用媒体源原始链接 / path = 总是优先用媒体源原始链接（.strm 网盘直链）/ emby = 总是用 Emby 直连', cur.urlFrom);
        if (from !== null) dpStore.set(dpKeys.urlFrom, (from.trim().toLowerCase() || 'auto'));
        const now = dpConf();
    ...
    async function dpTestConnection() {
        ...
        const jr = await dpHttp(`${conf.jriver}/MCWS/v1/Alive${jrQuery ? '?' + jrQuery : ''}`, { timeout: 3000 });
        ...
        if (jr.ok) {
            ...
            const idx = await dpHttp(`${conf.jriver}/MCWS/v1/${jrAuth.toString() ? '?' + jrAuth : ''}`, { timeout: 3000 });
            const fns = [...new Set(idx.text.match(/(?:Playback|Playlist|Control|Library|Browse|File)\/[A-Za-z]+/g) || [])].sort();
            fnList = fns.length ? `\n\nMCWS 函数 ${fns.length} 个：\n${fns.join('  ')}`
                : `\n\n/MCWS/v1/ 没解析出函数（HTTP ${idx.status}）：${(idx.text || '').replace(/\s+/g, ' ').slice(0, 300)}`;
        }
        alert(`开关 = ${dpEnabled() ? '开启' : '关闭'}，模式 = ${conf.mode}\n`
            ...
            + `MPC-HC 不通：MPC-HC 要先启动，且选项 → 网页界面 勾选监听端口。` + fnList);
    }
    const dpItemIdFromHash = h => (String(h || '').match(/[?&]id=([\w-]+)/) || [])[1] || null;
    ...
    function dpButtonBaseClass(cls) {
        const keep = String(cls || '').split(/\s+/).filter(c => c && !/^btn(Play|Resume|Download|More)$/i.test(c));
        return keep.length ? keep.join(' ') : 'raised button-submit block emby-button';
    }
    ...
    function dpRelayHref(mode, url, resumeMs) {
        ...
        return `${scheme}:${encodeURIComponent(url)}` + (resumeMs >= 30000 ? `||${Math.round(resumeMs)}` : '');
    }
    ...
    async function dpPlayWith(mode) {
        dpModeOverride = mode;
        try { await dpPlayFromPage(); } finally { dpModeOverride = null; }
    }
    ...
    function dpSyncDetailButtons() {
        ...
        if (!id) { dpRemoveDetailButtons(); return; }
        // 拿原生 ▶ 按钮当锚点：它在哪，「播放按钮旁边」就是哪
        const play = document.querySelector('.mainDetailButtons button.btnPlay, .detailButtons button.btnPlay, button.btnPlay')
            || document.querySelector('.mainDetailButtons button, .detailButtons button');
        ...
        while (/btn(Play|Resume|Download)/i.test((anchor.nextElementSibling || {}).className || '')) anchor = anchor.nextElementSibling;
        for (const [mode, label, icon] of dpPlayers) {
            ...
            btn.addEventListener('click', ev => {
                ...
                ev.stopPropagation();
                btn.disabled = true;
                dpPlayWith(mode).catch(e => { logger.error('直连: 按钮播放异常', e); alert('直连播放出错：' + e); })
    ...
    async function dpButtonCheck() {
        const ver = (typeof GM_info !== 'undefined' && GM_info.script) ? GM_info.script.version : '未知';
        const id = dpItemIdFromHash(location.hash);
        const sel = '.mainDetailButtons button.btnPlay, .detailButtons button.btnPlay, button.btnPlay';
        const play = document.querySelector(sel) || document.querySelector('.mainDetailButtons button, .detailButtons button');
        const row = play && play.parentElement;
        const have = () => dpPlayers.filter(([m]) => document.getElementById('etlp-dp-btn-' + m)).length;
        const lines = [
        ...
        if (row) lines.push('行内子元素：' + [...row.children].map(c => (c.id || (c.className || '').split(' ')[0] || c.tagName) + '“' + (c.textContent || '').trim().slice(0, 6) + '”').join(' | '));
